...
inline static __device__ float Euclidean(const float3 x1, const float3 x2)
{
    return sqrtf((x1.x - x2.x) * (x1.x - x2.x) + (x1.y - x2.y) * (x1.y - x2.y) + (x1.z - x2.z) * (x1.z - x2.z));
}

inline static __device__ float Euclidean3(const float4 x1, const float4 x2)
{
    return sqrtf((x1.x - x2.x) * (x1.x - x2.x) + (x1.y - x2.y) * (x1.y - x2.y) + (x1.z - x2.z) * (x1.z - x2.z));
}
...
inline static __device__ __host__ float3 uchar4_to_float3(const uchar4 c)
{
...
inline static __device__ float3 srgb2rgb(const float3 c)
{
...
inline static __device__ float3 rgb2xyz(const float3 c)
{
...
inline static __host__ float3 h_rgb2xyz(const float3 c)
{
    return make_float3(0.4124564f * c.x + 0.3575761f * c.y + 0.1804375f * c.z,
                       0.2126729f * c.x + 0.7151522f * c.y + 0.0721750f * c.z,
                       0.0193339f * c.x + 0.1191920f * c.y + 0.9503041f * c.z);
...
inline static __device__ float3 rgb2hsl(const float3& c)
{
...
inline static __device__ float3 xyz2lab(const float3 c)
{
...
inline static __host__ float3 h_xyz2lab(const float3 c)
{
    // assuming whitepoint D65, XYZ=(0.95047, 1.00000, 1.08883)
    float3 r = make_float3(c.x / 0.95047f, c.y, c.z / 1.08883f);
...
inline static __device__ float rgb2gray(const uchar4 c)
{
    return 0.2989f * (float)c.x + 0.5870f * (float)c.y + 0.1140f * (float)c.z;
}

inline static __device__ float distColor(const uchar4 cf1, const uchar4 cf2)
{
 ...
 */
inline static __device__ float CostYK(const int dx, const int dy, const uchar4 c1, const uchar4 c2, const float gammaC,
                               const float gammaP)
{
 ...
 * @see CostYK
 */
inline static __device__ float CostYKfromLab(const int dx, const int dy, const float4 c1, const float4 c2, const float gammaC,
                                      const float gammaP)
{
    // Euclidean distance in Lab, assuming linear RGB
...
inline static __device__ float CostYKfromLab(const float4 c1, const float4 c2, const float gammaC)
{
    // Euclidean distance in Lab, assuming linear RGB
    const float deltaC = Euclidean3(c1, c2);
    // const float deltaC = fmaxf(fabs(c1.x-c2.x),fmaxf(fabs(c1.y-c2.y),fabs(c1.z-c2.z)));
